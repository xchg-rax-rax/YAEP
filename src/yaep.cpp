...
void parse_elf_header(char* file_image, S* elf_header){
    /*
     *    As 64 bit and 32 bit headers are parse identically we simple call parse_elf_32_header 
     *    casting elf_header to a Elf32_Ehdr pointer.
     */
    ...
    printf("Magic Number:              %x %x %x %x", elf_header->e_ident[EI_MAG0], 
...
//        case ELFOSABI_NONE:
    ...
    printf("Number of Program Headers: ",elf_header->e_phnum);
    if (elf_header->e_phnum != PN_XNUM) {
        printf("%d\n",elf_header->e_phnum);
    }
    else {
        printf("%d\n",((T*)(file_image+elf_header->e_phoff))->sh_info);
    }
    ...
    printf("Number of Section Headers: ",elf_header->e_shnum);
    if (elf_header->e_shnum != 0) {
        printf("%d\n",elf_header->e_shnum);
    }
    else {
        printf("%d\n",((T*)(file_image+elf_header->e_shoff))->sh_size);
    }
    ...
    if (elf_header->e_shstrndx != SHN_XINDEX) {
        ...
        printf("%d\n",((T*)(file_image+elf_header->e_shoff))->sh_link);
    }
...
void parse_program_header(char* file_image, S* elf_header, int index) {
    /* 
    ...
    if (index >= elf_header->e_phnum) {
        printf("[!] Fatal Error. Cannot read program header %d, only %d exist.", 
                index+1, elf_header->e_phnum);
        exit(-1);
    }

    // get program header @ index
    U* program_header = (U*) (file_image + elf_header->e_phoff 
    ...
    switch (program_header->p_type) {
        ...
        default:
            if (PT_LOOS <= program_header->p_type && program_header->p_type <= PT_HIOS)
                printf("OS Specific\n");
            else if (PT_LOPROC <= program_header->p_type && program_header->p_type <= PT_HIPROC)
                printf("Processor Specific\n");
            else if (PT_LOSUNW<= program_header->p_type && program_header->p_type <= PT_HISUNW)
                printf("Sun Microsystems Specific\n");
            else
    ...
    if (program_header->p_align == 0 || program_header->p_align == 1) {
...
template <typename S, typename T>
void parse_section_header(char* file_image, S* elf_header, int index) {
    /* 
    ...
    T* section_header = (T*) (file_image + elf_header->e_shoff 
                                                + (elf_header->e_shentsize * index));

    // sh_name
    // Need to resolve name via the string table section header entry
    //printf("Section Name: %s",file_image+elf_header->e_b):
    ...
    switch(section_header->sh_type){
        ...
        case SHT_GNU_versym:
            ...
            break;
        default:
            if (SHT_LOSUNW <= section_header->sh_type && section_header->sh_type <= SHT_HISUNW)  {
                printf("Sun Specific Section (Unknown)\n");
            }
            else if (SHT_LOOS <= section_header->sh_type && section_header->sh_type <= SHT_HIOS) {
                printf("OS Specific Section (Unknown)\n");
            }
            else if (SHT_LOPROC <= section_header->sh_type && section_header->sh_type <= SHT_HIPROC) {
                printf("Processor Specific Section (Unknown)\n");
            }
            else if (SHT_LOUSER <= section_header->sh_type && section_header->sh_type <= SHT_HIUSER) {
    ...
    printf("Entry Size:        %d bytes",section_header->sh_entsize);
...
int main(int argc, char** argv) {
    ...
    if (file_image[4] == 0x01) {
        ...
        parse_elf_header<Elf32_Ehdr, Elf32_Shdr>(file_image, elf_32_header);
        // Parse Program Headers (if any)
        printf("Program Headers\n");
        printf("===============\n\n");
        for (i = 0; i < elf_32_header->e_phnum; i++)
            parse_program_header<Elf32_Ehdr, Elf32_Phdr>(file_image, elf_32_header, i);

        // Parse Section Headers (if any)
        printf("Section Headers\n");
        printf("===============\n\n");
        for (i = 0; i < elf_32_header->e_shnum; i++)
            parse_section_header<Elf32_Ehdr, Elf32_Shdr>(file_image, elf_32_header, i);
    }
    else if(file_image[4] == 0x02) {
        // get elf header
        elf_64_header = (Elf64_Ehdr*) file_image;
        // Parse as a 64 bit ELF
        parse_elf_header<Elf64_Ehdr, Elf64_Shdr>(file_image, elf_64_header);
        // Parse Program Headers (if any)
        printf("Program Headers\n");
        printf("===============\n\n");
        for (i = 0; i < elf_64_header->e_phnum; i++)
            parse_program_header<Elf64_Ehdr, Elf64_Phdr>(file_image, elf_64_header, i);

        // Parse Section Headers (if any)
        printf("Section Headers\n");
        printf("===============\n\n");
        for (i = 0; i < elf_64_header->e_shnum; i++)
            parse_section_header<Elf64_Ehdr, Elf64_Shdr>(file_image, elf_64_header, i);
    }
    else {
